      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          {products.slice(0, visibleCount).map((product) => (
            <div
              key={product.id}
              className={`bg-white rounded-xl shadow-sm p-4 hover:shadow-md transition relative group ${layout === "list" ? "flex items-center gap-4" : ""
                }`}
            >
              <div className={`relative ${layout === "list" ? "w-32 h-32" : "w-full"}`}>
                <img
                  src={product.image}
                  alt={product.title}
                  className={`rounded-lg object-contain ${layout === "list" ? "w-32 h-32" : "w-full h-48 mb-4"
                    }`}
                  onError={(e) => {
                ...
                {/* Action Icons */}
                <div
                  className={`absolute ${layout === "list" ? "top-0 left-0" : "top-0 "
              ...
              <div className={`${layout === "list" ? "flex-1" : ""}`}>
                <h2 className={`font-semibold text-gray-800 ${layout === "list" ? "text-lg" : "text-sm line-clamp-2"}`}>
                  {product.title}
                </h2>
